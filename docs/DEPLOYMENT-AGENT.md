...
**Via GitHub Actions** : un tag `vX.Y.Z` poussé sur un commit `main` déclenche `.github/workflows/release.yml`
- **même tag, même pipeline que le serveur** (voir [Déployer le serveur](DEPLOYMENT-SERVER.md)) :
  un seul tag publie à la fois l'image Docker du backend et les 4 archives agent — serveur et agent
  partagent désormais le même numéro de version, plus d'espace de tags séparé (`agent-vX.Y.Z` a été
  abandonné)
...
- création d'une **seule Release GitHub combinée** (contrainte GitHub : un tag = une release), nommée `Cerebro vX.Y.Z`, contenant les 4 archives agent en assets et le lien vers l'image Docker (le job de tests en amont sert de garde-fou : rien n'est publié s'il ne passe pas)

Deux jobs se déclenchent ensuite (même workflow, `needs: release`) :
- `publish-npm` republie le wrapper npm (`packaging/npm/`) avec le même numéro de version, via
  ...
  republier le wrapper seul, sans nouvelle release).
...
publication par `release.yml`), avec des champs `serverUrl`/`certThumbprint` à `null` par
